...
# [DOCUMENT B 추가] — 감사 기반 8개 보완 항목

---

### B19. 스쿼드 시너지 규칙

**목적:** 4명 조합이 수치에 영향을 준다. "팀 구성이 결과를 만든다" 체감의 핵심.  
**적용 시점:** 배틀 시작 시 1회 계산 → BattleState에 주입.

```ts
// src/lib/battle/synergy.ts
interface SynergyBonus {
  id: string
  label: string
  effect: (bs: BattleState) => void
}

function calcSynergy(squad: AgentInstance[]): SynergyBonus[] {
  const roles = squad.map(a => a.functionalRole)
  const bonuses: SynergyBonus[] = []

  // SCOUT + EXECUTOR: EXECUTOR 망설임 감소
  if (roles.includes('SCOUT') && roles.includes('EXECUTOR')) {
    bonuses.push({
      id: 'RECON_SYNC',
      label: 'RECON SYNC — EXECUTOR 망설임 -8%',
      effect: (bs) => { bs.executor.hesitateBonus = (bs.executor.hesitateBonus ?? 0) - 0.08 }
    })
  }

  // ANALYST + RISK: 전체 conf 보정
  if (roles.includes('ANALYST') && roles.includes('RISK')) {
    bonuses.push({
      id: 'RISK_FRAMEWORK',
      label: 'RISK FRAMEWORK — 전체 conf +5%',
      effect: (bs) => { bs.allies.forEach(a => { a.conf = Math.min(1, a.conf + 0.05) }) }
    })
  }

  // 4역할 전부 다를 때: 시작 OBJ 보너스
  const uniqueRoles = new Set(roles)
  if (uniqueRoles.size === 4) {
    bonuses.push({
      id: 'FULL_FORMATION',
      label: 'FULL FORMATION — 시작 OBJ +5',
      effect: (bs) => { bs.obj = Math.min(100, bs.obj + 5) }
    })
  }
...
**표시 규칙:**
- 배틀 인트로 직후 시너지 배너 순차 표시 (1개당 1.2s)
- 시너지 없으면 배너 없음
- Reflection 카드에 "시너지 RECON_SYNC 발동" 표기

---

### B20. 메모리 Bank → 배틀 수치 영향

**목적:** 메모리가 쌓일수록 에이전트가 실제로 강해진다. "훈련이 결과를 바꾼다" 검증 포인트.  
**적용 시점:** 배틀 시작 직전 `applyMemoryBonus()` 1회 실행.

```ts
// src/lib/battle/memory.ts
function applyMemoryBonus(agent: AgentInstance): MemoryBonusResult {
  const mem = agent.memoryBank
  if (mem.length === 0) return { confMod: 0, hesiteMod: 0, recallIntensity: 0 }

  // 승리 비율 → conf 보정 (최대 +0.10)
  const winRatio = mem.filter(m => m.outcome === 'WIN').length / mem.length
  const confMod = Math.min(0.10, winRatio * 0.12)

  // 같은 Doctrine으로 이긴 기록 2회 이상 → hesitate 감소
  const docWins = mem.filter(m => m.doctrine === agent.doctrine && m.outcome === 'WIN')
  const hesiteMod = docWins.length >= 2 ? -0.05 : 0

  // RECALL Phase 강도 (메모리 20개 = full intensity)
  const recallIntensity = Math.min(1, mem.length / 20)

  return { confMod, hesiteMod, recallIntensity }
}

// battleStore 초기화 시 사용
// agent.conf += memBonus.confMod
// agent.hesitateBonus += memBonus.hesiteMod
// bs.recallIntensity = memBonus.recallIntensity
```

**시각화:**
- recallIntensity > 0.5 → RECALL Phase 펄스 링 2개 (기본 1개)
- recallIntensity = 1.0 → ANALYST 머리 위 메모리 카드 아이콘 표시
- Reflection에 "메모리 보너스 conf +X%" 표기

---

### B21. 에이전트 생성 플로우

**목적:** 처음에 어떻게 에이전트를 만드는지 UI 단계를 명확히 정의.  
**라우트:** `/roster` → 생성 → `/agent/[id]`

**생성 단계 (5단계):**
```
Step 1: 이름 입력
  - 미입력 → AGENT_${nanoid(4).toUpperCase()} 자동
  - 중복 금지 (rosterStore 기준)
  - 최대 12자

Step 2: FunctionalRole 선택 (4개 카드)
  SCOUT / ANALYST / RISK / EXECUTOR
  → 선택 시 기본 ExpressionRole 자동 매핑:
    SCOUT → RIDER, ANALYST → ORACLE, RISK → GUARDIAN, EXECUTOR → PUSHER

Step 3: ExpressionRole 수동 변경 (선택사항)
  → 8개 중 선택 가능 (Lv 10 이후)
  → MVP에서는 자동 매핑만

Step 4: Doctrine 선택 (4개 카드)
  → 기본값: AGGRESSIVE
  → 카드에 수치 표시 (hesitate % / objPerHit)

Step 5: 생성 확인
  → 초기값 설정:
    { level: 1, xp: 0, bond: 10, conf: 0.5, hp: 100,
      memoryBank: [], hesitateBonus: 0,
      dataBinding: 'PRICE_ONLY', retrievalPolicy: 'RECENT' }
  → rosterStore.addAgent(newAgent)
  → navigate('/agent/[id]')
```
...
최대 보유: 12명
스쿼드 슬롯: 4명 (12명 중 선택)
BaseModel: MVP는 Qwen3-1.7B 1종만
동일 이름 금지 → 에러 메시지: "이미 같은 이름의 에이전트가 있습니다"
```

---

### B22. TrainingLoadout → 배틀 수치 변환

**목적:** DataBinding/RetrievalPolicy 선택이 배틀 conf/hesitate/RECALL에 실제로 영향.  
**적용 시점:** 배틀 시작 직전, applyMemoryBonus() 이후 추가 적용.
...
export const RETRIEVAL_POLICIES = {
  RECENT: {
    label: '최근 기억 우선',
    hesiteMod: -0.03,
    recallDepth: 1,
...
**최종 hesitateChance 계산 순서:**
```ts
function calcHesitate(agent: AgentInstance): number {
  const base = DOCTRINE[agent.doctrine].hesitate
  const memBonus = applyMemoryBonus(agent).hesiteMod
  const loadoutBonus = RETRIEVAL_POLICIES[agent.retrievalPolicy].hesiteMod
  const synergyBonus = agent.hesitateBonus ?? 0
  return Math.max(0.02, Math.min(0.90, base + memBonus + loadoutBonus + synergyBonus))
}
// 최솟값 0.02 보장 (0이 되면 절대 망설임 없음 → 밸런스 붕괴)
// 최댓값 0.90 보장 (1이 되면 항상 망설임 → 게임 불가)
```
...
**목적:** "이 에이전트에게 어떤 데이터를 줄지, 어떻게 기억을 검색할지" 설정하는 화면.  
**라우트:** `/lab`  
**Sprint 3에서 구현.**

**MVP에서 보여줄 것:**
```
① 선택된 에이전트 (squadStore.selected 기준)
② DataBinding 선택 카드 3종 (현재 선택 강조)
   - 잠긴 것은 dimmed + "Lv N 필요" 표시
③ RetrievalPolicy 선택 카드 3종 (현재 선택 강조)
④ 메모리 카드 목록 (최근 5개 미리보기)
   - 각 카드: outcome(WIN/LOSS) + lesson 1줄 + doctrine
   - 삭제 버튼 (×)
⑤ "메모리 전체 초기화" 버튼 (확인 모달 필요)
```

**MVP에서 보여주지 않을 것:**
```
- 파인튜닝 실행 버튼 (mlx-lm 미연결)
- 벤치마크 비교
- 프롬프트 스택 에디터
- 데이터소스 커스텀 추가
```
...
```ts
// 변경 즉시 저장 (autosave)
agent.dataBinding = selected
agent.retrievalPolicy = selected
rosterStore.updateAgent(agent.id, { dataBinding, retrievalPolicy })
// localStorage cogochi.roster.v2 에 반영
```

---

### B24. 배틀 결과 → 메모리 Writeback

**목적:** 배틀이 끝나면 레슨 카드가 에이전트에 자동 저장. "학습이 누적된다" 루프의 핵심.  
**실행 시점:** Reflection 카드 표시 직전 (결과 계산 직후).

```ts
// src/lib/battle/writeback.ts
interface MemoryRecord {
  ...
  cardsUsed: number
  lesson: string          // Reflection에서 생성된 1줄 요약
  stageId: string
  bondAtTime: number      // 압축 시 우선순위 기준
}

function writebackMemory(
  agent: AgentInstance,
  bs: BattleState,
  lesson: string
): void {
...
// 호출 위치: reflectionService.classify() 직후
// writebackMemory(executor, bs, reflectionNote.lesson)
```

**표시:**
- Reflection 카드 하단에 "메모리 카드 저장됨 (${agent.memoryBank.length}/50)" 표시
- 50개 도달 시 "오래된 기억 1개 삭제됨" 표시

---

### B25. 시나리오 패킷 구조

**목적:** 배틀 시작 데이터를 구조화. marketRegime이 지형과 수치에 영향.  
**생성 시점:** 스테이지 선택 → 독트린 선택 → 배틀 진입 직전.
...
interface ScenarioPacket {
  ...
  seedRandom: number              // 스테이지별 고정 seed
  marketRegime: MarketRegime
}

// 스테이지별 marketRegime 고정값 (config.ts)
export const STAGE_REGIME: Record<string, MarketRegime> = {
  s1: 'TRENDING_UP',
  s2: 'RANGING',
  s3: 'TRENDING_UP',
  s4: 'VOLATILE',
...
// marketRegime 효과 (배틀 시작 시 적용)
export const REGIME_EFFECT = {
  TRENDING_UP: {
    doctrineHint: 'AGGRESSIVE',   // Reflection에서 "이 지형엔 이 독트린" 힌트
    objFloor: 10,                 // 시작 OBJ 최소값
    rivalAtkMult: 1.0
  },
  TRENDING_DOWN: {
    doctrineHint: 'CONTRARIAN',
    objFloor: 0,
    rivalAtkMult: 1.2             // 적군 반격 20% 강화
  },
  RANGING: {
    doctrineHint: 'DEFENSIVE',
    objFloor: 0,
    hesiteGlobalMod: +0.05        // 전체 hesitate +5%
  },
  VOLATILE: {
    doctrineHint: 'NONE',
    objFloor: 0,
    chaosMod: true                // 매 라운드 conf ±0.05 랜덤 변동
  }
} as const
```
...
**목적:** 모델이 코딩 전에 스스로 7개를 확인하게 강제. 구조 오염 방지.  
**위치:** [SPRINT COMMAND] 바로 앞에 붙인다.

```
[세션 시작 전 자가 체크 — 아래 7개 확인 후 코딩 시작]

□ 1. 현재 스프린트 번호와 목표가 [SPRINT COMMAND]에 명시되어 있는가?
□ 2. 이전 스프린트 완료 조건이 전부 ✅인가?
     → 아니면 미완료 항목 먼저 해결
□ 3. 이번에 만들 파일의 의존 파일(types.ts, config.ts)이 먼저 존재하는가?
     → 없으면 의존 파일 먼저 생성
□ 4. Svelte 5 runes 문법을 쓰는가? ($state/$derived/$effect)
     → writable() 사용 금지
□ 5. 수치를 하드코딩하지 않고 config.ts에서 import하는가?
     → 숫자 리터럴이 로직 파일에 직접 있으면 config.ts로 이동
□ 6. Canvas arc() 드로잉 전 Math.max(0, ...) 래핑이 있는가?
     → 없으면 반드시 추가
□ 7. AgentInstance를 배틀 중 직접 변경하지 않는가?
     → rosterStore.updateAgent()를 통해서만 변경

7개 전부 ✅ 확인 후 → STEP 1(설계 검토)부터 순서대로 진행
```

---

*추가됨: 2026-03-08 — DocAudit 기반 8개 보완 항목 (B19~B26)*
