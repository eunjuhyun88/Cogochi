<script lang="ts">
  ...
  const { entries, selectedIds, onToggle } = $props<{
    entries: AiMonDexEntry[];
    selectedIds: string[];
    onToggle: (id: string) => void;
  }>();

  const synergy = $derived(summarizeTeamSynergy(selectedIds));
</script>
...
<section class="builder">
  <div class="builder-head">
    <div>
      <h2>Team Builder</h2>
      <p>최대 4종까지 선택됩니다. 4종 초과 선택 시 가장 오래된 슬롯이 교체됩니다.</p>
    </div>
    <div class="summary">
  ...
  <div class="grid">
    {#each entries as entry (entry.id)}
      <AiMonCard entry={entry} selected={selectedIds.includes(entry.id)} onSelect={onToggle} />
    {/each}
  </div>
</section>
